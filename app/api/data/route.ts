...
export async function POST(req:Request){
  ...
  try{const body=JSON.parse(raw);if(!Array.isArray(body.people)||!Array.isArray(body.projects))throw new Error("invalid data");const updatedAt=await writeState({version:2,people:body.people,projects:body.projects});return NextResponse.json({ok:true,updatedAt})}catch(error){console.error(error);return NextResponse.json({error:"Не удалось сохранить данные"},{status:500})}
}
