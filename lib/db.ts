...
export async function writeHistory(snapshots:Snapshot[]){const sql=await ensure();if(!sql)throw new Error("DATABASE_URL is not configured");for(const snapshot of snapshots){if(!/^\d{4}-\d{2}-\d{2}$/.test(snapshot.date)||!Array.isArray(snapshot.people))continue;const data=JSON.stringify({people:snapshot.people});await sql`INSERT INTO twinteem_snapshots (snapshot_date,data,updated_at) VALUES (${snapshot.date}::date,${data}::jsonb,now()) ON CONFLICT (snapshot_date) DO UPDATE SET data=EXCLUDED.data,updated_at=now()`}return snapshots.length}
