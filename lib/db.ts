...
export async function writeState(data:{people:unknown[];projects?:unknown[];version?:number}){const sql=await ensure();if(!sql)throw new Error("DATABASE_URL is not configured");const serialized=JSON.stringify(data),snapshot=JSON.stringify({people:data.people,projects:data.projects||[]}),today=dateKey();const rows=await sql`INSERT INTO twinteem_state (id,data,updated_at) VALUES ('main',${serialized}::jsonb,now()) ON CONFLICT (id) DO UPDATE SET data=EXCLUDED.data,updated_at=now() RETURNING updated_at`;await sql`INSERT INTO twinteem_snapshots (snapshot_date,data,updated_at) VALUES (${today}::date,${snapshot}::jsonb,now()) ON CONFLICT (snapshot_date) DO UPDATE SET data=EXCLUDED.data,updated_at=now()`;return rows[0]?.updated_at}
export async function readHistory(from:string,to:string){const sql=await ensure();if(!sql)return {configured:false,snapshots:[] as Snapshot[]};const rows=await sql`SELECT snapshot_date::text AS date,data,updated_at FROM twinteem_snapshots WHERE snapshot_date BETWEEN ${from}::date AND ${to}::date ORDER BY snapshot_date`;return {configured:true,snapshots:rows.map(row=>({date:row.date,people:Array.isArray(row.data?.people)?row.data.people:[],projects:Array.isArray(row.data?.projects)?row.data.projects:undefined,updatedAt:row.updated_at})) as Snapshot[]}}
export async function writeSnapshot(snapshot:Snapshot){const sql=await ensure();if(!sql)throw new Error("DATABASE_URL is not configured");const serialized=JSON.stringify({people:snapshot.people,projects:snapshot.projects||[]});const rows=await sql`INSERT INTO twinteem_snapshots (snapshot_date,data,updated_at) VALUES (${snapshot.date}::date,${serialized}::jsonb,now()) ON CONFLICT (snapshot_date) DO UPDATE SET data=EXCLUDED.data,updated_at=now() RETURNING updated_at`;return rows[0]?.updated_at}
