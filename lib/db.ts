...
export async function writeSnapshot(snapshot:Snapshot){const sql=await ensure();if(!sql)throw new Error("DATABASE_URL is not configured");const serialized=JSON.stringify({people:snapshot.people});const rows=await sql`INSERT INTO twinteem_snapshots (snapshot_date,data,updated_at) VALUES (${snapshot.date}::date,${serialized}::jsonb,now()) ON CONFLICT (snapshot_date) DO UPDATE SET data=EXCLUDED.data,updated_at=now() RETURNING updated_at`;return rows[0]?.updated_at}
